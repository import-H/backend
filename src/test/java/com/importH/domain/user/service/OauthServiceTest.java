...
class OauthServiceTest {
    ...
    void googleLogin_Success() throws Exception {
        ...
        given(oauthAdapter.getUserProfile(any(),any(),any())).willReturn(getSocialProfile());
    ...
    @Test
    @DisplayName("[성공] 구글로그인 및 계정 저장 - 이미 가입된 계정시 save 하지 않음")
    ...
    private User getUser() {
        return User.builder().email(getSocialProfile().getEmail()).role("ROLE_USER").build();
    }

    private SocialProfile getSocialProfile() {
        return SocialProfile.builder().email("test@mail.com").name("test").oauthId("1").build();
    }
